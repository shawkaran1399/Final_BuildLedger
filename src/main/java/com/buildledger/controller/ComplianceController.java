...
@Tag(name = "Compliance", description = "Compliance monitoring")
...
public class ComplianceController {
    ...
    @PatchMapping("/compliance-records/{complianceId}/status")
    @PreAuthorize("hasRole('COMPLIANCE_OFFICER') or hasRole('ADMIN')")
    @Operation(summary = "Update complianceRecord status", description = "COMPLIANCE_OFFICER/ADMIN: Progress compliance lifecycle")
    public ResponseEntity<ComplianceRecordResponse> updateComplianceStatus(
            @PathVariable Long complianceId,
            @RequestParam ComplianceStatus newStatus) {
        return ResponseEntity.ok(
                complianceService.updateComplianceRecordStatus(complianceId, newStatus)
        );
    }
}
